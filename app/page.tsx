...
const SuggestionButton: React.FC<SuggestionButtonProps> = ({ text, icon, color, onClick }) => (
  <motion.button
    ...
    whileTap={{ scale: 0.97 }}
    className="px-4 py-2 rounded-full bg-[#3567F5]/20 text-[#3567F5] hover:bg-[#3567F5]/10 text-sm flex items-center gap-2"
    onClick={onClick}
  >
...
const SendButton: React.FC<SendButtonProps> = ({ disabled, type = "submit" }) => (
  <motion.button
    ...
    type={type}
    disabled={disabled}
    className="rounded-full bg-[#3567F5] hover:bg-[#e95136] h-10 w-10 flex items-center justify-center text-white disabled:cursor-not-allowed"
  >
...
export default function Page() {
  ...
  const customSubmit = (e: FormEvent) => {
    ...
    // Get current message count from localStorage - using userMessageCountgem as the key
    const currentCount = parseInt(localStorage.getItem('userMessageCountgem') || '0');
    ...
    localStorage.setItem('userMessageCountgem', newCount.toString());
            ...
            {!hasMessages ? (
              ...
              <div className="py-20 text-center max-w-4xl mx-auto">
                <div className='flex flex-col sm:flex-row items-center size-fit gap-4 mx-auto h-fit mb-14'>
                  ...
                  >
                    <AuroraText className='text-3xl'>Justine</AuroraText>, l'assistante alumni de la GEM.
                  </motion.h2>
                  <video
                    src='/justine.mp4'
                    autoPlay
                ...
                >
                  <form ref={formRef} onSubmit={customSubmit} className="relative">
                    ...
                    <div className="absolute left-3 bottom-4 bg-[#3567F5]/20 text-[#3567F5] rounded-full px-3 py-1 flex items-center gap-2 text-sm pointer-events-none">
